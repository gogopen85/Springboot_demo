...
public class UserController {
    ...
    public ResponseEntity<?> register(@RequestBody UserRegistration userRegistration){
        ...
        else if("".equals(userRegistration.getUserId()))
        ...
        if(pattern.matcher(userRegistration.getUserId()).find())
            return res.resBadRequest("아이디에는 특수문자가 포함될 수 없습니다.");

        userService.save(new User(userRegistration.getUserId(), userRegistration.getName(),userRegistration.getPassword(), Arrays.asList(new Role("USER"), new Role("ACTUATOR"))));
